...
// Production defaults. The API and the web dashboard live on different hosts
// (api.* vs app.*), so the dashboard URL is its own setting, never derived.
export const DEFAULT_MCP_URL = "https://api.agentlooker.ai/mcp";
export const DEFAULT_DASHBOARD_URL = "https://app.agentlooker.ai/dashboard";
...
export const DASHBOARD_URL = pick(ENV_DASHBOARD_URL) ?? cfg.dashboardUrl ?? DEFAULT_DASHBOARD_URL;
